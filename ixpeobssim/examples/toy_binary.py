...
import ixpeobssim.config.toy_binary as input_model
...
def simulate():
    ...
    file_list = pipeline.xpphase(*file_list, parfile=file_path, suffix='folded')
    file_list = pipeline.xpophase(*file_list, parfile=file_path,
                                  bary=True, ra=18.14, dec=-35.02, suffix='folded')
...
def display_pulse_profile():
    ...
    model_pp = input_model.pl_norm(phase)
...
def display_pol_degree():
    ...
    for i, (min_, max_) in pairwise_enum(ENERGY_BINNING):
        ...
        plt.plot(phase, numpy.degrees(input_model.pol_ang(energy, phase)),
                 color=last_line_color(), label=model_label(energy))
    setup_gca(ymin=50, ymax=80, legend=True, **fmtaxis.pp_pol_ang)
...
if __name__ == '__main__':
    pipeline.bootstrap_pipeline('toy_binary')
