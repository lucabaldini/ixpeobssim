...
* The current implementation of the X-ray intrumental background is based on the
...
* To our knowledge, this background component is stable over
  ...
  rate decrease over multiple yearsmay be expected. It is always a good idea to 
...
this case, we provide the means to model itwith a linear function of the 
...
* The flares can then be estimated in the full detector area and subtracted from the data.
  This relies on the assumption that the flaring activity is unuiform across the detector,
